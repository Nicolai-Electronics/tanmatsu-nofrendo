...
void initRomList() {
    ...
    // Check if the roms file was successfully opened
    if (romsFile == NULL) {
        ESP_LOGE(TAG,
                 "Error opening file '%s'\n"
                 "Please checkout the README.md of the project on how to create this file.",
                 ROM_LIST);
        while (1) {
            // TODO: Add error handling here
            // sleep for a while to keep the watchdog happy
            vTaskDelay(1000);
        }
    }
    ...
    for (int i = 0; i < entryCount; i++) {
        ...
        fscanf(romsFile, "%d.\t%c\t%[^\t]\t%[^\n\r]\n", &menuEntries[i].entryNumber, &menuEntries[i].icon,
               menuEntries[i].name, menuEntries[i].fileName);
