...
static int32_t decay_lut[16];
static int     vbl_lut[32];
static int     trilength_lut[128];

/* noise lookups for both modes */
#ifndef REALTIME_NOISE
static int8_t noise_long_lut[APU_NOISE_32K];
static int8_t noise_short_lut[APU_NOISE_93];
...
/* DMC transfer freqs */
const int dmc_clocks[16] = {428, 380, 340, 320, 286, 254, 226, 214, 190, 160, 142, 128, 106, 85, 72, 54};
...
INLINE int8_t shift_register15(uint8_t xor_tap) {
...
#else  /* !REALTIME_NOISE */
static void shift_register15(int8_t* buf, int count) {
    static int sreg = 0x4000;
...
#define APU_MAKE_RECTANGLE(ch)                                                                                        \
    static int32_t apu_rectangle_##ch(void) {                                                                         \
        int32_t output, total;                                                                                        \
        int     num_times;                                                                                            \
                                                                                                                      \
        APU_VOLUME_DECAY(apu.rectangle[ch].output_vol);                                                               \
                                                                                                                      \
        if (false == apu.rectangle[ch].enabled || 0 == apu.rectangle[ch].vbl_length) return APU_RECTANGLE_OUTPUT(ch); \
                                                                                                                      \
        /* vbl length counter */                                                                                      \
...
*/
static int32_t apu_triangle(void) {
    APU_VOLUME_DECAY(apu.triangle.output_vol);

    if (false == apu.triangle.enabled || 0 == apu.triangle.vbl_length) return APU_TRIANGLE_OUTPUT;
...
static int32_t apu_noise(void) {
    ...
    if (false == apu.noise.enabled || 0 == apu.noise.vbl_length) return APU_NOISE_OUTPUT;
...
int apu_write(uint32_t address, uint8_t value) {
    int chan;
