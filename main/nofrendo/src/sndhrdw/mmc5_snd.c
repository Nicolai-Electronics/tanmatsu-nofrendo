...
static int32_t mmc5_rectangle(mmc5rectangle_t *chan)
{
   int32_t output;
...
static uint8_t mmc5_read(uint32_t address)
{
   uint32_t retval;
...
static int32_t mmc5_process(void)
{
   int32_t accum;
...
/* write to registers */
static uint32_t mmc5_write(uint32_t address, uint8_t value)
{
   int chan;
