...
#define WAVEDISP_WIDTH 128
    ...
    if (NULL == vis_buffer) {
        ...
        /* initial old value */
        if (16 == vis_bps)
            oldval = 0x40 - (((((uint16_t*)vis_buffer)[0] >> 8) ^ 0x80) >> 2);
        else
            oldval = 0x40 - (((uint8_t*)vis_buffer)[0] >> 2);

        for (loop = 1; loop < WAVEDISP_WIDTH; loop++) {
            // val = 0x40 - (vis_buffer[(uint32_t) (loop * scale)] >> 2);
            if (16 == vis_bps)
                val = 0x40 - (((((uint16_t*)vis_buffer)[(uint32_t)(loop * scale)] >> 8) ^ 0x80) >> 2);
            else
                val = 0x40 - (((uint8_t*)vis_buffer)[(uint32_t)(loop * scale)] >> 2);
            if (oldval < val) {
                offset     = oldval;
    ...
    /* solid wave */
        ...
        for (loop = 0; loop < WAVEDISP_WIDTH; loop++) {
            // val = vis_buffer[(uint32_t) (loop * scale)] >> 2;
            if (16 == vis_bps)
                val = ((((uint16_t*)vis_buffer)[(uint32_t)(loop * scale)] >> 8) ^ 0x80) >> 2;
            else
                val = ((uint8_t*)vis_buffer)[(uint32_t)(loop * scale)] >> 2;
            if (val == 0x20)
