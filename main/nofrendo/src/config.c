...
static void my_destroy(myvar_t** var) {
    ...
    if ((*var)->group) free((*var)->group);
    if ((*var)->key) free((*var)->key);
...
static myvar_t* my_create(const char* group, const char* key, const char* value) {
    ...
    if ((var->group = malloc(strlen(group) + 1)) && (var->key = malloc(strlen(key) + 1)) &&
            ...
            do {
                ...
                switch (*s) {
                    ...
                    default:
                        ...
                        while (strlen(key) && isspace((unsigned char)key[strlen(key) - 1])) key[strlen(key) - 1] = '\0';

                        while (isspace((unsigned char)*s)) s++;

                        while (strlen(s) && isspace((unsigned char)s[strlen(s) - 1])) s[strlen(s) - 1] = '\0';
...
static const char* read_string(const char* group, const char* key, const char* def) {
    ...
    var = my_lookup(group, key);
    if (NULL == var) {
        if (def != NULL) write_string(group, key, def);
...
/* interface */
config_t config = {open_config, close_config, read_int, read_string, write_int, write_string, CONFIG_FILE};
