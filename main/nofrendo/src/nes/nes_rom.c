...
//     if (NULL == fp) return; /* no palette found  */
...
int rom_checkmagic(const char* filename) {
    ...
    if (0 == memcmp(head.ines_magic, ROM_INES_MAGIC, 4)) /* not an iNES file */
        return 0;
...
char* rom_getinfo(rominfo_t* rominfo) {
    ...
    log_printf(temp, " [%d] %dk/%dk %c", rominfo->mapper_number, rominfo->rom_banks * 16, rominfo->vrom_banks * 8,
