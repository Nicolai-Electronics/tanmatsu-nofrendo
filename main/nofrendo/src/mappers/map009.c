...
static void map9_write(uint32_t address, uint8_t value) {
    switch ((address & 0xF000) >> 12) {
        ...
        case 0xB:
            regs[0] = value;
            if (0xFD == latch[0]) mmc_bankvrom(4, 0x0000, value);
            break;

        case 0xC:
            regs[1] = value;
            if (0xFE == latch[0]) mmc_bankvrom(4, 0x0000, value);
            break;

        case 0xD:
            regs[2] = value;
            if (0xFD == latch[1]) mmc_bankvrom(4, 0x1000, value);
            break;

        case 0xE:
            regs[3] = value;
            if (0xFE == latch[1]) mmc_bankvrom(4, 0x1000, value);
            break;
...
static map_memwrite map9_memwrite[] = {{0x8000, 0xFFFF, map9_write}, {-1, -1, NULL}};
