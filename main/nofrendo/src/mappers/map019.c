...
#define N_BANK1(table, value)                                                                                          \
    {                                                                                                                  \
        if ((value) < 0xE0)                                                                                            \
            ppu_setpage(                                                                                               \
                1, (table) + 8,                                                                                        \
                &mmc_getinfo()->vrom[((value) % (mmc_getinfo()->vrom_banks * 8)) << 10] - (0x2000 + ((table) << 10))); \
        else                                                                                                           \
...
static map_memwrite map19_memwrite[] = {{0x5000, 0x5FFF, map19_write}, {0x8000, 0xFFFF, map19_write}, {-1, -1, NULL}};
