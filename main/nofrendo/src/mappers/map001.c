...
static void map1_write(uint32_t address, uint8_t value) {
    ...
    switch (regnum) {
        ...
        case 2:
            if (regs[0] & 0x10) mmc_bankvrom(4, 0x1000, value);
            break;
...
static void map1_init(void) {
    ...
    if (mmc_getinfo()->rom_banks == 0x20) mmc_bankrom(16, 0xC000, 0x0F);
...
static map_memwrite map1_memwrite[] = {{0x8000, 0xFFFF, map1_write}, {-1, -1, NULL}};
